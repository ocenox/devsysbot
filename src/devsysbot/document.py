...
def _agent_tasks(a: dict[str, Any]) -> str:
    ...
    if _g(a, "project.source", "").startswith("Existing"):
        app_steps = [f"Clone `{_g(a, 'project.git_url')}` into `/code/{name}` (dev stage)."]
    else:
        app_steps = [f"Scaffold the dev stage for `{name}` ({_g(a, 'project.stack')})."]
    if _yes(a, "docker.enabled"):
...
def build_add_project(answers: dict[str, Any], secret_keys: list[str]) -> str:
    """Build a focused, idempotent 'add a project' document for the add-project skill."""
    name = _g(answers, "project.name", "newapp")
    desc = _g(answers, "project.description") or "(no description provided)"
    today = date.today().isoformat()

    if _g(answers, "project.source", "").startswith("Existing"):
        source_line = f"Clone `{_g(answers, 'project.git_url')}` into `/code/{name}`."
        source_summary = f"existing repository `{_g(answers, 'project.git_url')}`"
    else:
        source_line = f"Scaffold a `{_g(answers, 'project.stack')}` app in `/code/{name}`."
        source_summary = f"a new {_g(answers, 'project.stack')} project"

    port = _g(answers, "addproj.port") or "the next free port"
    steps = [source_line]
    if _yes(answers, "addproj.db"):
        steps.append(f"Create database `{name}_dev`"
                     + (f" and `{name}_build`" if _yes(answers, "stages.build") else "")
                     + " on the existing host DB **only if it does not already exist**.")
    steps.append(f"Add the dev stage stack (bind-mount `/code/{name}`, port {port}, wire DB/mail hosts).")
    if _yes(answers, "stages.build"):
        steps.append("Add the build stage (CI-built image) for this app.")
    if _yes(answers, "addproj.subdomain"):
        steps.append("Add `<service>-<stage>` subdomains + reverse-proxy entries **if not already present**; "
                     "reload the proxy.")
    if _yes(answers, "addproj.mail"):
        steps.append("Provision a mail account for this app **if a mail server exists and the account is missing**.")
    if _yes(answers, "addproj.ci"):
        steps.append("Derive a CI pipeline (lint/test/deploy) from the existing template, adjusted to this app.")
    steps.append("Run the `generate-dashboard` skill to add this app's swimlane.")

    task_md = "\n".join(f"- [ ] {s}" for s in steps)

    return f"""# Add Project — {name}

> Generated by DevSysBot on {today}. Hand this to a coding agent and run the
> **`add-project`** skill. It adds {source_summary} to the **existing** environment and
> pulls in only the infrastructure that is **not already present** (idempotent).
...
- The base environment already exists (ZFS `/code`, host DB, reverse proxy, secret store).
- Reuse shared infrastructure; do not recreate anything that already exists.

## Task list (idempotent)

{task_md}

## Rules

- Feature branch only; never write to `main`.
- No hardcoded secrets — reference the secret store by name.
- Never use the former product brand name in generated files.

## Acceptance

- [ ] The new app is reachable over TLS for each stage.
- [ ] Existing apps are unaffected; no infrastructure was duplicated.
- [ ] The dashboard shows the new app.
"""
